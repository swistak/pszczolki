/****************************************************************************
** Meta object code from reading C++ file 'BackgroundBee.h'
**
** Created: Sat Jul 3 04:56:45 2010
**      by: The Qt Meta Object Compiler version 62 (Qt 4.6.2)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "BackgroundBee.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'BackgroundBee.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.6.2. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_BackgroundBee[] = {

 // content:
       4,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       2,       // signalCount

 // signals: signature, parameters, type, tag, flags
      45,   15,   14,   14, 0x05,
      82,   72,   14,   14, 0x05,

 // slots: signature, parameters, type, tag, flags
     113,   14,   14,   14, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_BackgroundBee[] = {
    "BackgroundBee\0\0currentIteration,currentPrice\0"
    "finishedIteration(int,int)\0aprox,bee\0"
    "finishedWorking(double,double)\0"
    "stopWorking()\0"
};

const QMetaObject BackgroundBee::staticMetaObject = {
    { &QThread::staticMetaObject, qt_meta_stringdata_BackgroundBee,
      qt_meta_data_BackgroundBee, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &BackgroundBee::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *BackgroundBee::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *BackgroundBee::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_BackgroundBee))
        return static_cast<void*>(const_cast< BackgroundBee*>(this));
    return QThread::qt_metacast(_clname);
}

int BackgroundBee::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QThread::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: finishedIteration((*reinterpret_cast< int(*)>(_a[1])),(*reinterpret_cast< int(*)>(_a[2]))); break;
        case 1: finishedWorking((*reinterpret_cast< double(*)>(_a[1])),(*reinterpret_cast< double(*)>(_a[2]))); break;
        case 2: stopWorking(); break;
        default: ;
        }
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void BackgroundBee::finishedIteration(int _t1, int _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void BackgroundBee::finishedWorking(double _t1, double _t2)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)), const_cast<void*>(reinterpret_cast<const void*>(&_t2)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}
QT_END_MOC_NAMESPACE
